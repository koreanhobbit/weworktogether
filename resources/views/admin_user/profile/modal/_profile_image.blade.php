				...
				@slot('panelBody')
					...
					<div class="tab-content">
						<div class="tab-pane fade" id="profileImageTab">
							@include('admin_user.profile.partial._profile_image')
						</div>
						<div class="tab-pane fade active in" id="uploadProfileImageTab">
							<div class="row">
								<div class="col-sm-12">
									<form action="{{ route('image.store') }}" class="dropzone" id="addNewProfileImageDz" data-url="{{ route('user.profile.index', ['user' => Auth::user()->id, 'name' => Auth::user()->name]) }}">
										{{ csrf_field() }}
										<div class="fallback">
